...
static int atecc_i2c_probe(struct i2c_client *client,
  ...
  uint8_t i;
  int ret = -1;
  uint8_t i2c_wake_msg =
      0x00;  // msg sent to I2C bus to wake up ATECC608A from sleep state
...
ssize_t devAttrAteccSerial_show(struct device *dev,
  ...
  if (atecc.probed) {
    return sprintf(
        ...
        atecc.serialNumber[0], atecc.serialNumber[1], atecc.serialNumber[2],
        atecc.serialNumber[3], atecc.serialNumber[4], atecc.serialNumber[5],
        atecc.serialNumber[6], atecc.serialNumber[7], atecc.serialNumber[8]);
  }
