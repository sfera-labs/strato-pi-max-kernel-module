...
static const char VALS_SD_SD1_ENABLED[] = { 4, '0', '1', '_', '2' };
...
static struct DeviceAttrBean devAttrBeansSd[] = {
...

	{
		...
		.vals = VALS_SD_SD1_ENABLED,
	},

	{
		.devAttr = {
			.attr = {
				.name = "sd_main_default",
				.mode = 0660,
			},
			.show = devAttrI2c_show,
			.store = devAttrI2c_store,
...
static void _i2c_unlock(void) {
	mutex_unlock(&_i2c_mtx);
}
	...
	for (i = 0; i < 10; i++) {
		...
		if (res == len + 1) {
			...
			if (crc == buf[len]) {
				...
				pr_notice(
						LOG_TAG "i2c read error: crc=0x%x crc_rcv=0x%x\n",
						buf[len], crc);
				res = -1;
			}
		}
...
static int64_t _i2c_write_no_lock(uint8_t reg, uint8_t len,
...
static int64_t _i2c_read_segment(uint8_t reg, uint8_t len,
...
static int64_t _i2c_write_segment(uint8_t reg, uint8_t len,
		uint32_t mask, uint8_t shift, uint32_t val, bool readBack) {
	int64_t res = 0;
	uint8_t i;
...
static ssize_t devAttrI2c_show(struct device *dev,
	...
	res = _i2c_read_segment(reg, specs->len,
...
static ssize_t devAttrConfig_store(struct device *dev,
	...
	res = _i2c_write_no_lock(I2C_REG_USER_CFG_CMD, 2, 0x2a00 | val,
			0);
	if (res > 0) {
		for (i = 0; i < 10; i++) {
			msleep(1);
...
static ssize_t devAttrBlink_store(struct device *dev,
	...
	res = _i2c_write_segment(dab->regSpecs.reg, dab->regSpecs.len,
			dab->regSpecs.mask, dab->regSpecs.shift,
			on, true);
	if (res > 0) {
		res = _i2c_write_segment(dab->regSpecs.reg + 1, dab->regSpecs.len,
				dab->regSpecs.mask, dab->regSpecs.shift,
				off, true);
		if (res > 0) {
...
#else
static int _i2c_probe(struct i2c_client *client, const struct i2c_device_id *id) {
#endif
	uint8_t i;
	int64_t res;
	bool ok;
...
static int _device_add(struct platform_device *pdev, struct DeviceBean *db,
	...
	while (db->devAttrBeans[ai].devAttr.attr.name != NULL) {
		...
		do {
			if (dab->bitMapLen > 0) {
				dabM = devm_kzalloc(
						&pdev->dev, sizeof(struct DeviceAttrBean), GFP_KERNEL);
				if (!dabM) {
					return -ENOMEM;
				}
			...
			if (device_create_file(dev, &dabM->devAttr)) {
				pr_err(LOG_TAG "failed to create device file '%s/%s' s=%d\n",
						db->name, dabM->devAttr.attr.name, (expbIdx + 1));
				return -1;
			}
			fi++;
